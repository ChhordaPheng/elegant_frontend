...
const debouncedApplyFilters = debounce(async () => {
  await topTrendingStore.applyFilters(currentFilters.value);
}, 500); // 500ms delay
...
const clearAllFilters = async () => {
  ...
  await topTrendingStore.applyFilters({});
};
...
const applySorting = async (
  ...
  await topTrendingStore.applyFilters(filtersWithSort);
};

// Handle pagination
const handlePageChange = async (newPage: number) => {
  page.value = newPage;
  await topTrendingStore.setPage(newPage);
};
...
const handleItemsPerPageChange = async (count: number | string) => {
  ...
  else if (count === "all") perPage = topTrendingStore.total; // Large number for "all"

  topTrendingStore.perPage = perPage;
  ...
  await topTrendingStore.applyFilters(filtersWithPerPage);
};
    ...
    <v-app>
        ...
        <v-container fluid class="pa-0 mt-[60px]">
          <v-row no-gutters>
            ...
            <v-col cols="4" class="pa-3 pl-12" style="min-height: 100vh">
              ...
              <div
                v-for="topTrending in topTrendings.slice(0, 7)"
                :key="topTrending.id"
                ...
                <v-container class="pa-0 pr-12">
                  <v-row>
                    <v-col>
                      <div class="pl-3">
                        <p>
                          Clothing ({{ topTrendings?.length || 0 }}
                          items)
                        </p>
                  ...
                  <div class="d-flex flex-wrap">
                    <!-- Loading skeleton -->
                    <div v-if="topTrendingStore.isLoading" class="w-full">
                      <v-row>
                    ...
                    <div
                      v-else-if="
                        !topTrendingStore.hasItems &&
                        !topTrendingStore.isLoading
                      "
                    ...
                    <v-card
                      v-else
                      v-for="bestSeller in topTrendings"
                      :key="bestSeller.id"
                  ...
                  <v-row justify="center">
                    <v-col cols="8">
                      <v-container class="max-width">
                        <v-pagination
                          :model-value="topTrendingStore.page"
                          :length="topTrendingStore.totalPages"
                          rounded="circle"
                          class="my-4"
      ...
      <template v-else>
        ...
        <v-main>
          <v-container>
            <div class="pl-3">
              <p>Clothing ({{ topTrendings?.length || 0 }} items)</p>
              <div class="my-3 flex items-center flex-wrap">
            ...
            <div class="d-flex flex-wrap justify-center">
              ...
              <div
                v-else-if="
                  !topTrendingStore.hasItems && !topTrendingStore.isLoading
                "
                class="w-full text-center py-8"
              >
                <v-icon
